...
public class ImageHelper {
	...
	public static byte[] getImageData(String imagePath, int bytesPerPixel, int width, int height, int rotation,
		...
		return getImageData(imagePath, bufferedImage, bytesPerPixel, width, height, rotation, roundsOfDarkness);
	}

	public static byte[] getImageData(String imageName, BufferedImage bufferedImage, int bytesPerPixel, int width,
			int height, int rotation, int roundsOfDarkness) throws IOException {
		byte[] imageData = new byte[width * height * bytesPerPixel];

		if (bufferedImage.getWidth() > width || bufferedImage.getHeight() > height) {
			throw new IllegalStateException(
					format("Image {0} ({1}x{2}) is larger than {3}x{4}!", imageName, bufferedImage.getWidth(),
							bufferedImage.getHeight(), width, height));
		}

		System.out.println(format("Image {0} loaded with W {1} and H {2}", imageName, bufferedImage.getWidth(),
	...
	/**
	 * Image is read to byte array pixel per pixel for each row to get one continuous line of data. But a different
	 * ordering is needed in case a matrix is wired in columns, first column down, second column up, third column
	 * down,...
	 * <p>
